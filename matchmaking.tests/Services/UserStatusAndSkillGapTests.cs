...
public sealed class UserStatusAndSkillGapTests
{
    ...
    private sealed class FakeCompanyService : ICompanyService
    {
        ...
        public FakeCompanyService(ICompanyRepository companyRepository) => _companyRepository = companyRepository;
    ...
    private sealed class FakeUserStatusMatchRepository : IUserStatusMatchRepository
    {
        ...
        public FakeUserStatusMatchRepository(IReadOnlyList<Match> matches) => _matches = matches;

        public IReadOnlyList<Match> GetByUserId(int userId) => _matches.Where(match => match.UserId == userId).ToList();

        public IReadOnlyList<Match> GetRejectedByUserId(int userId) => _matches.Where(match => match.UserId == userId && match.Status == MatchStatus.Rejected).ToList();
    }
